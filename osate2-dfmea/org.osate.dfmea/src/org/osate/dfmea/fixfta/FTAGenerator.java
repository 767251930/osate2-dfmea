package org.osate.dfmea.fixfta;
...
//	}
	...
	public String getinstancename(EObject a) {
		...
		if (a instanceof ErrorBehaviorState) {
			result = "ErrorBehaviorState";
		} else if (a instanceof ErrorBehaviorEvent) {
			result = "ErrorBehaviorEvent";
		} else if (a instanceof CompositeState) {
			result = "CompositeState";
		} else if (a instanceof ErrorEvent) {
			result = "ErrorEvent";
		} else if (a instanceof ErrorSource) {
			result = " ErrorSource";
		} else if (a instanceof ErrorPropagation) {
			result = "ErrorPropagation";
		} else if (a instanceof CompositeState) {
			result = "ErrorPropagation";
		} else if (a instanceof ConditionExpression) {
	...
	private Event finalizeAsOrEvents(ComponentInstance component, Element ne, TypeToken type, List<EObject> subEvents) {
		if (subEvents.size() == 0) {
			return null;
		}
...
//		removeZeroOneEventSubGates(rootevent);
	...
	private Event minimalCutSet(Event rootevent) {
		...
		Event alternatives = FaultTreeUtils.createUniqueIntermediateEvent(ftaModel,
				...
				(TypeToken) rootevent.getRelatedErrorType());
...
//		}
	...
	@Override
	protected EObject processErrorBehaviorState(ComponentInstance component, ErrorBehaviorState state, TypeToken type,
			BigDecimal scale) {
...
//----  FIX FTA BUG
	...
	@Override
	protected EObject processTransitionCondition(ComponentInstance component, ErrorBehaviorState source, TypeToken type,
			EObject conditionResult, EObject stateResult, BigDecimal scale) {
